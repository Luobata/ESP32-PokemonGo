...
class Party:
    ...
    def to_bytes(self) -> bytes:
        """定长布局：1 + 1 + 6×12 + 151×12 = 1886 字节。

        与 `load()` 的 `need`、模块尾的 `SERIALIZED_BYTES` 同源 —— 三个数字
        必须一起变。曾漏算 6×12=72 的队伍区把总数写小：照抄注释写固件
        会得到错位寻址，而错位后读出的仍是合法数值，不报错只是离谱。

        仓库那 151 格**按物种号定位**：第 i 格就是 species_id = i+1，
        空格全零。定长而非「只存已拥有的」：
          · 固件侧读写是 (base + (sid-1)*12) 一次寻址，无需遍历
          · 掉电时半写状态更容易恢复（每格独立，不依赖前面的格子）
          · 省下的字节没有意义 —— 满编 1886 B 在 8MB 里是 0.02%
        """

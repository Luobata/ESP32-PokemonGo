# S4 养成状态机

> 当前规则（2026-09-13）：全局体能每小时恢复100点，约36秒1点，从零约1小时回满；开机及熄屏待机恢复，完全关机暂不补算。

> 2026-09-11：探索改为只扣5点体能，旧探索次数1:1保留为可选探索情报，零情报也可探索；体能每小时恢复50点，约2小时回满。规则及验证见 reports/exploration-intel-2026-09-11.md。

三条状态轴随时间推进；饱食或心情低于阈值进入消沉、降低战斗能力。全局体能作为探索、训练和玩耍的行动预算，不再影响战斗能力。各轴最低为 0；不清档，也不删除伙伴。

## 输入 / 输出

| 方向 | 来源 / 目标 | 字段 |
|---|---|---|
| 入 | 时间 | `ts`（只需要「过了多久」） |
| 入 | `sim/sensing.py` 的 `SensingResult` | `motion_events`（增量）、`is_new_place` |
| 入 | 绝对时间 | `is_night`（唯一需要真实时钟的输入） |
| 入 | S9 道具 | 喂食消耗品 |
| 入 | S3 未获胜的战斗终局 | 同场一次：体能 −20、心情 −15 |
| 入 | S16 训练家挑战入场 | 切磋 −5、道馆/赤红 −10、联盟整轮 −20；提交成功后扣除 |
| 入 | S16 训练家战败 | 同场一次：心情 −5，体能不追加扣除；认输不退入场消耗 |
| 出 | S2 捕获 | `catch_window_bonus` 直接乘判定窗口宽度 |
| 出 | S3 战斗 | `ability_factor`（消沉 0.6） |
| 出 | S7 进化 | `intimacy`、`explore_value` |
| 出 | S6 存档 | 整个 `PetState` |

## 核心逻辑

**已实现**。参考算法在 `sim/gameplay.py` 的 `PetState`；固件使用 `nurture.c` 的 Q10 状态，world 是动作与保存的唯一所有者。

| 行为 | 函数（均在 `sim/gameplay.py`） |
|---|---|
| 时间推进 | `PetState.advance(ts, motion_events, is_night)` |
| 喂食 | `PetState.feed(amount=30.0)` |
| 玩耍 | `PetState.play()` |
| 休息 | `PetState.rest(hours=8.0)` |
| 战败 | `PetState.defeat()`；固件由 `world_apply_defeat_uid()` 调用 `nurture_defeat()` |
| 到新地方 | `PetState.on_new_place()` |
| 久别重逢 | `PetState.on_reunion(days_away)` |
| 移动量事件 | `PetState.on_motion_event()` |
| 派生状态 | `is_despondent` / `ability_factor` / `catch_window_bonus` / `mood_label` |

衰减常量已在 `sim/gameplay.py` 定稿：

| 常量 | 值 |
|---|---|
| `SATIETY_DECAY_PER_HOUR` | 4.0 |
| `MOOD_DECAY_PER_HOUR` | 3.0 |
| `STAMINA_RECOVER_PER_HOUR` | 12.0（设备暂未接入昼夜判定，实际始终按此速度） |
| `STAMINA_COST_PER_MOTION_EVENT` | 0 |
| `LOW_THRESHOLD` | 25.0 |
| `DESPONDENT_PENALTY` | 0.6 |
| 野战战败体能扣减（固件 `NURT_DEFEAT_STAMINA`） | 20 |
| 野战战败心情扣减（固件 `NURT_DEFEAT_MOOD`） | 15 |
| 训练家战败心情扣减（固件 `NURT_CHALLENGE_DEFEAT_MOOD`） | 5 |

**不做惩罚性死亡。** 饱食或心情低于 25 进入 `is_despondent`，
主宠攻击/特殊攻击能力打折。固件新会话传入 `nurture_ability_factor()`：消沉为 614/1024，否则 1024/1024。
首次战斗、逃跑或实际投球前再取当前养成快照，此后同场保留该系数。
饱食和心情恢复到 25 或以上后，新战斗使用正常系数，即使体能为 0。所有扣减钳位在 0–100，不触发清档。

战败不扣经验、饱食或亲密度。`world_apply_defeat_uid()` 先提交候选存档，成功后才发布养成扣减和本场标记；失败不处罚，重试不重复扣减。
败战结果页 A 进入照料：玩耍心情 +15/体能 −5，喂食饱食 +30/心情 +5，均钳位到 100。“恢复说明”仅显示提示，不即时补充体力。

体力只随设备运行时间恢复，熄屏仍计时；完全关机期间暂不补算。探索成功消耗 5 点，体力不足或保存失败不扣。能量根改为饱食 +10、心情 +20；牛奶为饱食 +20、心情 +10，均不恢复养成体力。对战中恢复 HP 与此独立。

经验倍率为 `80 + (饱食百分比 + 心情百分比)/5 + 亲密百分比/10`，整数运算，范围 80–130%。野生对战基础经验为 `等级×8+20`：获胜 100%、失败 30%、捕获成功 60%，再乘养成倍率。战胜后捕获可追加捕获经验；逃跑没有经验。野生经验给队首，训练家对战按既有参战分配规则；奖励与存档提交绑定，失败重试不重复发放。

稀有加成为 `(饱食百分比 + 心情百分比 + 亲密百分比)/6` 千分点，最高 50：三星以上累计概率最多提高 5 个百分点、四星以上 2 个百分点、五星 0.5 个百分点。仍受剧情门槛约束，不改变宝可梦自身稀有度或闪光判定。照料页显示经验倍率和稀有加成。

**RTC 漂移影响很小。** `advance()` 只用 `(ts - _last_ts) / 3600.0`，
即「过了多久」。漂移会让流逝时长有偏差，但不会让状态机错乱。
只有 `is_night` 需要绝对时间；若对时方案都不理想，砍掉夜间加速即可
（[02-sensing.md#25](../02-sensing.md#25-无连接带来的时间问题)）。

**原型跑出的一条结论**：一天照料一次抵不过衰减，三次（每 8 小时一个窗口）
才打得平 —— 见 `sim/prototype.py` 的 `care_slot` 逻辑。这不是拍的，是跑出来的。

## 数据结构

当前固件 `nurture_t` 保存四个 `int32_t` Q10 值（饱食、心情、体能、亲密度）和 `int64_t last_us`，
由现有 V14 `save_t` 的 `pet` 字段保存；物种、经验和队伍仍用各自字段。挑战费用和战败扣减只更新已有养成轴，不改变存档版本。

以下为早期 Python 原型的 24 字节布局草案，不是当前固件的读写格式：

```
pet_state 24 B
   off  size  field
   0    1     species_id      1~151
   1    1     type_name_idx   TYPES 下标 0~14
   2    1     satiety         定点：0~200（值 = 浮点 × 2），0.5 精度
   3    1     mood            同上
   4    1     stamina         同上
   5    1     intimacy        同上
   6    2     explore_value   u16，够存 65535 次移动量事件
   8    4     last_ts         对应 _last_ts
   12   8     nickname        UTF-8 截断（"小家伙" = 9 B → 需 12 B，见「疑问」）
   20   4     reserved        预留：进化次数、累计照料次数
```

草案的 ×2 精度已由实际固件 Q10（×1024）取代；定点运算避免 C3 上的软件浮点开销。

## 与三键约束的关系

对应 **P5 照料页**：**A 执行 / B 切换 / C 返回**。

B 键在「喂食 / 玩耍 / 恢复说明」三项间循环，A 执行当前项。
这是「少而重」（[04-gameplay.md#43](../04-gameplay.md#43-培育)）的直接体现。
满足条件时菜单追加进化。长按 A 切换官方/金银民间旧译；状态与提示常驻照料页底部，不改变养成数据。

主宠常驻 **P1 待机页**，用原品 back sprite（当前 48×48）+ `sim/effects.py` 的
`breath_sequence()` 呼吸。消沉时可切 `FLASH_DARK` 灰阶映射表压暗姿态，
零额外素材。

## 待 Phase 0 验证的参数

| 参数 | 依赖 |
|---|---|
| 全部六个衰减常量 | **RTC 走时精度** —— 若只能做到「天」级，小时级衰减率没有意义，三条轴要重新按天标定 |
| `is_night` 能否保留 | 外置 32.768kHz 晶振是否存在；无对时方案则砍掉夜间加速 |
| 一天几次照料才打平 | 已用合成数据跑出「三次」，需真实作息数据复核 |

08-systems.md 把 S4 标为「部分实现 / ⏳ RTC 精度」，与此一致。

## 边界情况

1. **`_last_ts` 为 `None`**（首次运行或存档新建）—— `advance()` 已处理：只记录 ts 直接返回，不推进。
2. **`ts` 回跳**（对时修正把时间调早）—— `advance()` 已处理：`hours <= 0` 直接返回。状态不会因对时倒退。
3. **长时间离线**（出差两周）—— `hours` 很大，三条轴会全部触底到 0 并进入消沉，但**不清零存档**。`on_reunion(days_away)` 给心情 +8 作为重逢补偿，奖励真实生活节律。
4. **`explore_value` 溢出** —— u16 上限 65535；按每天几十次事件算够几年，但仍需 clamp 而非回绕（回绕会让 S7 的进化条件突然失效）。
5. **定点精度下的衰减吞没** —— 若单次 `advance()` 的时长极短（几秒），衰减量小于 0.5 会被定点截断成 0，长期空转会让状态永不下降。对策：累加余量，或只在 ≥1 分钟间隔时推进。

## 疑问

`nickname` 的字节预算需要确认。`PetState.nickname` 默认值 `"小家伙"`
UTF-8 是 9 字节，上表给 8 字节存不下。三个方向：

- 扩到 12 字节（4 个汉字），`pet_state` 变 28 字节
- 存中文点阵字库的字索引而非 UTF-8（每字 2 字节，4 字仅 8 字节）——
  与「中文点阵字体子集化」（08-systems.md 实现顺序阶段 D）天然配套
- 昵称不可改则不存，编译期常量

倾向第二种，但它依赖 D 阶段的字库子集先定稿。这条不影响 S4 的逻辑，
只影响 S6 的总字节数（08-systems.md 估的约 710 字节需按此微调）。

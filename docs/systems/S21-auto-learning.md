...
核查日期：2026-09-09。本页早期基线为 166 招 / 缺 73 招；后续已扩展为 **191 招 / 缺 48 招**，详见 [S22](S22-move-acceptance.md)。

历史核查：结论：**尚未覆盖全部金银招式**。普通获得路线涉及 238 种招式；本轮从缺失 74 种减少为 73 种，引擎支持初代 165 招和新增潮旋。现有初代招式沿用本项目战斗规则，不能把“支持”理解为逐条完全复刻金银机制。
...
238 种中，按途径分别有升级 203、TM/HM 57、遗传 94 种（彼此重叠，不能相加）。事实快照保存在 `data/pokemon_moves/gold_silver.json`；核查脚本为 `tools/pipeline/audit_gold_silver_moves.py`。基线报告 `reports/evidence/gold-silver-2026-09-09/audit.json` 保留修改前 74 种缺失，下表更新为当前剩余的 48 种。
...
`verify_gs_learning.py` 检查 151 × 100 个物种等级组合、升级不遗忘、HM 资格与潮旋伤害/束缚；`verify_combat_fx.py` 当前检查全部 191 招、双侧与不同体型、34086 帧；`verify_trainer_sendout.py` 检查初始双方、主动换宠、倒下换宠和敌方替换，共 118 次画面一致性检查。

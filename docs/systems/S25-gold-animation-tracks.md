...
对照固定版本 `data/moves/animations.asm` 的 `BattleAnimSub_Beam`：破坏光线、极光束、日光束发射阶段使用同一段脚本，在 (64,92)、(80,84)、(96,76)、(112,68) 依次生成四个光束对象，随后生成前端闪光。原版光束有亮芯／描边，并按对象寿命依次出现、消失；它不是全程同时显示的一条无描边直线。

旧的竖屏映射对对象原点采用横纵不同的缩放，但对象内部仍固定2倍。单个对象形状保持了，跨对象连续光束却因此接缝错层、重叠分叉。这三招的光束改为在统一原版坐标空间投影每个像素边界，保持跨对象拼接连续。日光束蓄力仍沿用原来的独立粒子映射；其他188招不改动。未接入的空气爆炸也引用同类光束对象，将来接入时需要同样处理。

`verify_joined_beams.py` 独立先合成原版OAM整幅图，再逆向采样到竖屏，与生产C渲染比较，覆盖3招、双方、10个参数案例、442帧。这项检查验证连续光束几何，不是整个原作战斗画面的等价认证。全招式边界／恢复回归78536帧、原生预览6112次像素检查及固件构建通过。证据在 `reports/evidence/beam-alignment-2026-09-09`；其中 `original-oam-geometry.png` 为提取轨迹中的原坐标图块合成示意，并非原机截图。

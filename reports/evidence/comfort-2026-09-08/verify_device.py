...
try:
    ...
    read_for(max(0, 58 - (time.monotonic() - start)))
    before = status(); assert not before['off'] and before['brightness'] == 100
    read_for(max(0, 60.6 - (time.monotonic() - start)))
    asleep = status(); assert asleep['off'] and asleep['brightness'] == 0
    checks['automatic_timeout'] = {'before_60s': before, 'after_60s': asleep}
    print('60-second display timeout: PASS', flush=True)
    read_for(35)
    later = status()

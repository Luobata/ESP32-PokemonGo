...
`convert_font.py` 生成 **16×16 点阵子集**：

| 项 | 值 |
|---|---|
| 字符集 | **346 个**（209 宝可梦名 + 149 UI 文案，重叠 12） |
| 单字 | 32 B（16×16 1bpp） |
| 码点索引 | 692 B（346 × u16，有序，固件二分查找） |
| **合计** | **11.5 KB** |
| 对照 | GB2312 全字库 6700 字 = 209 KB，**子集省了 199 KB** |

**为什么是 16×16 而不是 12×12**：实测从 PingFang 渲染「妙」字，
12×12 笔画糊成一团、左右结构分不开；16×16 的左「女」右「少」清晰可辨。
体积只差 2.8 KB（209 字时 3.7 vs 6.5 KB），在 8MB flash 里是零头 ——
**清晰度值这个钱**。

UI 文案按页面组织在 `UI_STRINGS`，便于对照 `docs/08-systems.md` 增删。

> **这是管线里唯一需要第三方依赖的一步**（PIL/Pillow），

...
动作是整数算术（+30/+5/+15/−5/+1），从整数值起始状态出发
两边都落在精确值上，**差 1 就是错**：

    feed:  satiety +30（钳 100）、mood +5（钳 100）
    play:  体能至少 5 点才执行：mood +15、stamina −5、intimacy +1；
           心情和亲密度钳 100，体能不足时四条轴都不变
    rest:  不直接恢复体能；恢复只来自 tick 的实际时间推进

钳位边界必测：饱食 90+30 → 100；体能不足不能继续刷心情或亲密度。
完整的 5 点体能定点数边界还由 verify_world_party.py 覆盖。

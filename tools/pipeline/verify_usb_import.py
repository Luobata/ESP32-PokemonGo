...
static void init(void){transcript[0]=0;snapshots=staged=restarts=0;usb_backup_init(snapshot,emit,"001122334455","1111111111111111111111111111111111111111111111111111111111111111",15);usb_backup_restore_hooks(prepare,restart,0,0);cmd("HELLO");}
static void offer(void){char s[300];snprintf(s,sizeof(s),"!PWBACKUP OFFER %s 001122334455 %s 15 24576 %08x\n",session,"1111111111111111111111111111111111111111111111111111111111111111",restore_crc32(incoming,sizeof(incoming)));feed(s);}
...
for(const [key,value] of [['sha256','0'.repeat(64)],['crc32','0'.repeat(8)],['save_version',99],['firmware','dirty'],['nvs_offset',0],['payload_base64','YQ==']])await assert.rejects(()=>decodeBackup(JSON.stringify({...file,[key]:value}),webcrypto));

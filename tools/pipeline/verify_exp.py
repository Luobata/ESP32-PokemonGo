...
曲线是按实测产出校准过的基准（7 天约 15000 exp → 1 周 Lv18、
1 月 Lv29，见 `sim/systems.py` exp_for_level 上方注释），硬件侧要 1:1
复刻。抽样抓不住「一格错、整体偏一点」的缺陷：除数抄成 3、截断方向
...
## 覆盖
    ...
    · 溢出锚点       exp_for_level(100) = 2_500_000，远小于 uint32_t 上限
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        # 溢出锚点：Lv100 = 2_500_000，逐值相等已排除回绕，这里再钉死数值
        g100 = int(d.ask("fl 100"))
        if g100 != 2_500_000:
            fails.append(f"锚点 exp_for_level(100): C {g100} ≠ 2_500_000")
        print(f"  溢出锚点       exp_for_level(100) = {g100:,}，"
              f"uint32 上限 4,294,967,295（余量 ~{0xFFFFFFFF // 2_500_000}×）")

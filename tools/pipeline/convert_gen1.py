...
# ---------------------------------------------------------------------------
# 记录结构：定长 **32** 字节，固件可 (base + id*32) 直接索引
#
# 这个数字曾误写成 28（字段表与 RECORD_SIZE 都是对的，只有标题错）。
# 照那个数写固件会让第 2 条记录起就整体错位 4 字节 ——
# 而错位后读出的仍是合法数值（种族值、属性 id 都在 0~255 内），
# 不会崩，只会全都是错的。所以特意标一下：末尾的 8 字节 reserved
# 也在 32 之内（24 + 8 = 32），别把它当成 28 之外的附加区。
#
#   off  size  field

#!/usr/bin/env python3
"""A continuous adventure edit from native game frames and the production mixer."""
...
ap=argparse.ArgumentParser();ap.add_argument('--check-only',action='store_true');ap.add_argument('--out',type=Path,default=ROOT/'reports/video/pokewalk-story-2026-09-09');args=ap.parse_args();out=args.out;out.mkdir(parents=True,exist_ok=True)
FPS=30
scenes=[
 dict(name='opening',act=1,title='故事，从一个邀请开始。',sub='大木博士正在等你。',duration=6,boot='0 25 12 74 3 123 0 0 0',events={120:['key 2 1']}),
 dict(name='starter',act=1,title='皮卡丘，就决定是你了。',sub='选好第一位伙伴，我们出发。',duration=6,events={30:['key 1 1'],60:['key 1 1'],90:['key 1 1'],135:['key 0 1']}),
 ...
 dict(name='meet',act=2,title='原来是尼多娜。',sub='试着邀请它，加入我们的冒险。',duration=5,events={0:['key 0 1']}),
 dict(name='capture',act=2,title='看准时机，投出精灵球。',sub='等它安静下来……我们成功了！',duration=7,setup=['key 0 1'],events={45:['key 0 1']}),
 ...
 dict(name='gym',act=4,title='一起赢下，第一场道馆战。',sub='道馆战斗节选 · 出场、交锋，直到分出胜负。',duration=18,setup=['page 13','key 0 1'],battle=True),
 dict(name='victory',act=4,title='第一枚徽章，属于我们。',sub='胜利经验也分享给了没有上场的皮卡丘。',duration=5),
 ...
 dict(name='epilogue',act=6,title='而在成为冠军之后……',sub='通关后的故事 · 熟悉的道馆，迎来更强的挑战。',duration=8,boot='13 25 80 74 3 123 0 0 0',setup=['party_add 30 80 80 1000 0','challenge_unlock 8191'],events={45:['key 1 1'],90:['key 0 1']}),
 dict(name='outro',act=6,title='你的下一次相遇，会是谁？',sub='PokeWalk · 把伙伴和冒险，带在身边。',duration=4,setup=['page 1']),
]
DURATION=sum(s['duration'] for s in scenes);ACTS=['初遇','出发','成长','挑战','新旅程','未完待续']
...
try:
 for s in scenes:
  ...
  begin=r.inspect();frames=[];tail=deque(maxlen=270);checks=0;source_frames=0
  for f in range(5400 if s.get('battle') else s['duration']*FPS):
   ...
   source_frames=f+1
   if s.get('battle'):
    if f<270:frames.append(sample)
    else:tail.append(sample)
    if f%3==0 and r.inspect()['challenge']['mode']==5:
     assert r.inspect()['challenge']['won'] and len(tail)==270
     frames.extend(tail);break
   else:frames.append(sample)
  assert len(frames)==s['duration']*FPS,(s['name'],len(frames))
  end=r.inspect()
  ...
  if s['name']=='epilogue':assert len(end['challenge']['sides'][1]['mons'])==6
  ...
  manifest['scenes'].append({'name':s['name'],'act':s['act'],'start':start,'duration':s['duration'],'source_seconds':source_frames/FPS,'source_cuts':jump_at,'checks':checks,'rms':round(rms,2),'music_ids':sorted({x[2] for x in frames if x[2] is not None}),'party_before':begin['party'],'party_after':end['party'],'defeated':end['challenge']['defeated'],'exploration':end['exploration']})
...
assert global_frame==DURATION*FPS and len(pcm)==DURATION*22050*2

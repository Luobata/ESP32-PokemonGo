  ...
  accept(line) {
    ...
    if(p[1]==='BEGIN') {
      ...
      if(!/^[1-9][0-9]{0,9}$/.test(id)||!(/^[a-f0-9]{12}$/.test(device))||
         !/^[a-f0-9]{64}$/.test(firmware)||version!=='15'||address!=='36864'||size!=='24576'||!(/^[a-f0-9]{8}$/.test(crc))) throw Error('设备或存档版本不支持');
...
export async function decodeBackup(text,cryptoAPI=globalThis.crypto) {
  ...
  if(d.format!=='pokewalk-nvs-backup'||d.format_version!==1||d.chip!=='esp32c3'||d.save_version!==15||d.nvs_offset!==36864||d.nvs_size!==SIZE||

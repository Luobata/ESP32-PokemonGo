...
(async function(){
 ...
 let catalog=[],sources=null,session=null,frame=0,frameCount=36,playing=true,busy=false,timer=null,queue=Promise.resolve();
 ...
 async function request(action,args={}){
  ...
  const count=Number(res.headers.get('X-Preview-Animation-Frames'));if(count>0){frameCount=count;frame=Math.min(frame,frameCount-1);$('frame').max=frameCount-1;}
 ...
 async function render(){if(!session||!selected())return;await request('move_preview',{move:selected().id,side:Number($('side').value),mode:Number($('mode').value),frame});$('frame').value=frame;$('frame-label').textContent=frame+' / '+(frameCount-1);}
 function info(){const m=selected();$('meta').textContent=m?`${types[m.type]} · 威力 ${m.power||'—'} · 命中 ${m.accuracy===255?'必中':m.accuracy+'%'}\n动画：${m.animation==='属性/状态共用动画'?m.animation:'招式指定动作'}${m.new?' · 本批新增':''}\n${m.animation_note||'项目适配动画，尚未逐帧对齐原版'}`:'无匹配招式';sourceInfo();}
 function sourceInfo(){
  const row=sources?.moves.find(m=>m.id===selected()?.id);$('source-details').hidden=!row;if(!row)return;
  $('source-link').href=row.source_url;$('source-link').textContent='查看金银原作脚本 · '+row.script;
  $('source-summary').textContent=`原作依赖：${row.objects.length} 种对象、${row.functions.length} 种运动、${row.background_effects.length} 种背景效果。还原状态：原作脚本尚未完整移植。`;
 ...
 $('restart').onclick=()=>{frame=0;enqueue(render);};$('step').onclick=()=>{playing=false;$('play').textContent='播放';frame=(frame+1)%frameCount;enqueue(render);};
 ...
 timer=setInterval(()=>{if(!playing||busy||document.hidden||!session)return;busy=true;enqueue(async()=>{frame=(frame+1)%frameCount;await render();}).finally(()=>busy=false);},90);
})();

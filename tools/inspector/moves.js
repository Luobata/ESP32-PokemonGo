...
(async function(){
 ...
 async function request(action,args={},expected=null){
  ...
  const bytes=new Uint8Array(await res.arrayBuffer());if(expected!==null&&expected!==revision)return false;
  const count=Number(res.headers.get('X-Preview-Animation-Frames'));if(count>0){frameCount=count;frame=Math.min(args.frame??frame,frameCount-1);$('frame').max=frameCount-1;}

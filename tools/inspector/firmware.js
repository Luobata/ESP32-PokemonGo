...
(function(){
  ...
  function suspendClock(){clockAt=performance.now();clockEpoch++;clearTimeout(timer);timer=null;clockStatus();}
  ...
  function send(action,args={},guard=null){
    // Preserve 上/下/确认 while a tick is in flight. Each input sees the session and
  ...
  async function perform(action,args){
    ...
    try{
      ...
      $('status').textContent=(screenOff?'已熄屏 · 按任意功能键亮屏 · ':'')+`${lastPage} · ${ms} ms · 构建 ${version}`+(mismatch>=0?` · 横带重绘差异 ${mismatch} 像素`:'');
  ...
  async function changeScene(action,args){
    ...
    finally{sceneChanges--;clockAt=performance.now();scheduleClock();}
  }
  ...
  // The inspector's selector loads a scene. In-game 上/下/确认 navigation preserves
  ...
  async function pump(){
    ...
    const now=performance.now(),ms=Math.min(60000,Math.max(0,Math.floor(now-clockAt)));clockAt=now;
    try{await send('tick',{ms},()=>epoch===clockEpoch&&canRun());}
    finally{pumpActive=false;scheduleClock();}
  }
  $('play').onclick=()=>{paused=!paused;suspendClock();scheduleClock();};
  // Match BSP events, including instant PRESS for capture timing. A completed
  // DOWN hold owns its release; it must never also navigate to the next row.
  ...
  function beginKey(id,key){
    ...
    if(held.event===1)send('key',{key,event:0});
  ...
  function endKey(id,commit){
    ...
    if(held.event===1)send('key',{key:held.key,event:4});
    if(commit&&!held.long)send('key',{key:held.key,event:held.event});
    if(held.event===1)send('key',{key:held.key,event:5});
  }
  ...
  window.addEventListener('keydown',e=>{
    ...
    const key=e.key==='ArrowUp'?0:e.key==='ArrowDown'?1:e.key==='Enter'?2:'abc'.indexOf(e.key.toLowerCase());

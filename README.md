...
**硬件未到手，Phase 0 实测阻塞。** 硬件规格已从主控型号与官方器件清单确认，
但四项关键数据必须实测。凡依赖它们的数值结论目前均为**待定**。

| 阻塞项 | 卡住了什么 |
|---|---|
| 续航天数 | 扫描频率上限 → 遭遇频率、地点时间分辨率 |
| 三态判别稳定性与转换误报率 | 猎场/基地结构能否成立 |
| RTC 走时精度 | 状态轴是「小时」级还是「天」级 |
| 口袋 RSSI 基线 | 匹配算法的权重曲线与阈值 |
| 真实环境 AP 密度 | biome 数量 → 所需美术资产量 |

本仓库当前内容是**硬件到手前可推进的工作**：仿真器、采集器、PC 端算法与玩法原型。
产出物在硬件到手后可直接复用，且不会因实测推翻设计而白做。

...
设备「菜单 → 选项」提供备份存档和导入存档。开发管理网页通过 USB 将备份保存到本地目录；导入需在设备确认，自动备份覆盖前的进度，再校验、暂存并重启恢复。devbox 只托管网页，存档不上传服务器。目前限同设备同固件构建，首次升级需更新分区表，详见 [存档管理工具](tools/save-manager/README.md)。

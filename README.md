...
- 四条探索路线：探索仅消耗体能；Wi-Fi 环境变化自动记录新地点并影响养成，体能随设备运行时间恢复。不是 GPS 定位，也不是实际计步。
...
主页同时显示当前等级的经验进度、统一体能和预计回满时间。升级后显示等级变化与新学会的招式；分享经验升级的队员也会提示，招式仍自动学习、永不遗忘。提示等战斗/捕获动画结束后出现，C 继续、长按 B 关闭，多页可用 A/B 翻阅。
...
挑战消耗全局体能：路线切磋 5 点，道馆/赤红 10 点，四天王与冠军整轮 20 点（后续关卡不重复扣）。探索仅消耗 5 点体能；已发现野怪的战斗/捕捉不额外收费。入场保存成功后才扣，认输不退；训练家战败仅额外扣 5 点心情。低体能不再降低战斗能力，饱食和心情仍影响养成。体能上限 100，开机/熄屏待机每小时恢复 100 点（约 1 小时回满），真正关机暂不恢复。详见[挑战体能与节奏验证](reports/challenge-stamina-2026-09-10.md)。
...
固件应用在 `firmware/build/PokeWalk.bin`，应用地址 `0x10000`。现有设备增量更新前备份 NVS，不能把应用文件当成从 `0x0` 烧写的合并固件。存档版本 V15，3728字节；真实设备备份、扫描原始数据与凭证不进入 Git。

AI Passport 社区项目为 `234`（PokeWalk），安装版本与审核状态请查看[创作者工作台](https://ai-passport.folotoy.cn/account/?project=234)与[发布回执](reports/evidence/community-publisher-2026-09-09/submission-receipt.json)。社区上传使用通过官方校验的 `0x0` 合并固件、3:4 封面和双语介绍；审核通过后才公开展示。
...
探索仅消耗5点体能，已取消额外情报点数；原情报加成合并进基础收益。体能约1小时回满，训练家换宠列表显示等级和HP。[当前规则](reports/stamina-only-2026-09-13.md)。

进化演出：等级、进化石与机器共用约8.5秒的原版八轮切换节奏和光球效果；成功后C继续，提交前长按B可取消。[实现与边界](reports/evolution-scene-2026-09-11.md)。

2026-09-13：取消探索情报点数、进度条和额外储备，探索与重复挑战统一消耗体能。原情报加成并入基础收益，约1小时恢复满体能。旧存档兼容，详见 [体能统一说明](reports/stamina-only-2026-09-13.md)。

森林秘境：菜单选择「秘境」，从自己的随行队伍中选 1～3 只伙伴出发。八节点连战、营地与路线分支、15 种局内强化，阶段经验只发给参战伙伴，道具进入正式背包，失败保留已结算收益。每次新开局消耗 20 体能，中途可继续。已修复点击出发时的任务栈溢出并完成真机验证。[9 月 13 日完整发布说明](reports/release-notes-2026-09-13.md) · [设计与范围](docs/design/roguelike-dungeon.md)

## 开发用 USB 存档备份与导入

设备「菜单 → 选项」提供备份存档和导入存档。[在线存档管理页](https://Luobata.github.io/ESP32-PokemonGo/)通过 USB 将备份保存到本地目录，无需启动本地服务；导入需在设备确认，自动备份覆盖前的进度，再校验、暂存并重启恢复。devbox 只托管网页，存档不上传服务器。目前限同设备同固件构建，首次升级需更新分区表，详见 [存档管理工具](tools/save-manager/README.md)。

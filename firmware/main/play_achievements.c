...
static void draw_all(void) {
 ...
 for(int y=0;y<SCREEN_H;y+=SCREEN_BAND_H){
  ...
  game_ui_text_centered(y,16,258,208,16,feedback?feedback:"长按B查看上一项",GAME_UI_MUTED);
  game_ui_footer(y,"[A]领取 [B]下一 [C]返回");screen_push_band(y);
 }
}
...
void play_achievements_key(bsp_btn_t b,bsp_btn_ev_t e){
 if(b==BSP_BTN_DOWN&&(e==BSP_BTN_CLICK||e==BSP_BTN_LONG)){selected=(selected+ACHIEVEMENT_COUNT+(e==BSP_BTN_LONG?-1:1))%ACHIEVEMENT_COUNT;feedback=0;}
 else if(e!=BSP_BTN_CLICK)return;
 else if(b==BSP_BTN_OK){nav_back(PAGE_MENU);return;}
 else if(b==BSP_BTN_UP){static const char *messages[]={"奖励已放入背包","尚未达成 继续冒险","奖励已经领取","道具已满 请先腾出空间","保存失败 按A重试"};feedback=messages[world_achievement_claim(selected)];}
